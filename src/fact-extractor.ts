...
// Uses LLM API to extract preferences, corrections, patterns, and facts
// Stores results via MemoryIndex — zero context window cost to main agent
import type { Logger, FactExtractorConfig } from "./types.js";
...
function buildExtractionPrompt(userName: string, agentName: string): string {
  return `You are a fact extraction system. Given a conversation between a user (${userName}) and an AI assistant (${agentName}), extract any notable facts, preferences, corrections, patterns, or decisions.
...
export class FactExtractor {
  ...
  private async maybeExtract(): Promise<void> {
    ...
    const conversation = turns.map(t => {
      const role = t.direction === "in" ? (t.sender ?? this.userName) : this.agentName;
      return `${role}: ${t.text}`;
    }).join("\n");

    try {
      const facts = await this.callExtractionModel(conversation);
      ...
      for (const fact of facts) {
        ...
        const slug = fact.key.endsWith(".md") ? fact.key : `${fact.key}.md`;
  ...
  /** Call extraction model API */
  private async callExtractionModel(conversation: string): Promise<ExtractedFact[]> {
    const prompt = buildExtractionPrompt(this.userName, this.agentName);
    ...
    try {
      ...
      this.logger.warn("FactExtractor: failed to parse extraction response", { text: text.slice(0, 200) });
      return [];
    }
  }
