// CRC: crc-SkillTransport.md | Seq: seq-skill-callback.md
...
// HTTP transport: skill has its own server, loop POSTs events to it
export class HttpSkillTransport extends SkillTransport {
  ...
  // Called by HttpApi when skill POSTs an event to the loop
...
// Stdio transport: child process communicates via JSON lines on stdin/stdout
...
// Direct transport: in-process function calls
export class DirectSkillTransport extends SkillTransport {
  ...
  // Called by in-process skill to emit event to the loop
