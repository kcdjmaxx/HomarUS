// Agent registry for tracking background agents spawned by Claude Code.
// Agents register via REST API, report completion via callback, and
// the registry emits events that the MCP event loop picks up.
...
export class AgentRegistry {
  ...
  private emit(type: string, agentId: string, description: string, extra: Record<string, string>): void {
    this.emitFn?.({
      id: uuid(),
      type,
