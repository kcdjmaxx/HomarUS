// Built-in tool: grep — search file contents with regex
...
export const grepTool: ToolDefinition = {
  name: "grep",
  description: "Search file contents using regex. Supports file type filtering, glob filtering, context lines, and multiple output modes (content, files_with_matches, count).",
  parameters: {
    type: "object",
    properties: {
      pattern: { type: "string", description: "Regex pattern to search for" },
      path: { type: "string", description: "File or directory to search in (default: working directory)" },
      glob: { type: "string", description: 'Glob to filter files (e.g. "*.ts", "**/*.test.js")' },
      type: { type: "string", description: "File type filter (js, ts, py, rust, go, java, md, json, yaml, html, css, sh)" },
      output_mode: { type: "string", description: "Output mode: content (matching lines), files_with_matches (file paths only), count (match counts)", enum: ["content", "files_with_matches", "count"] },
  ...
  async execute(params: unknown, context: ToolContext): Promise<ToolResult> {
    ...
    const files: string[] = [];
    const stat = await import("node:fs").then((fs) => fs.statSync(rootPath));
    if (stat.isFile()) {
      files.push(rootPath);
    } else {
    ...
    for (const file of files) {
      ...
      let content: string;
      try {
        const fstat = await import("node:fs").then((fs) => fs.statSync(file));
