// Built-in tools: memory_search, memory_get, memory_store
...
export function createMemoryTools(memoryIndex: MemoryIndex): ToolDefinition[] {
  const memorySearchTool: ToolDefinition = {
    name: "memory_search",
    description: "Search the memory index using hybrid vector + text search. Returns ranked results with file paths and content snippets.",
    parameters: {
      type: "object",
      properties: {
  ...
  const memoryGetTool: ToolDefinition = {
    name: "memory_get",
    description: "Retrieve the full content of a specific file from memory by its path.",
    parameters: {
      type: "object",
      properties: {
  ...
  const memoryStoreTool: ToolDefinition = {
    name: "memory_store",
    description: "Store content to a file in memory and index it for future search. Creates the file if it doesn't exist.",
    parameters: {
      type: "object",
      properties: {
