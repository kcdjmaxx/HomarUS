...
// Per-domain SQLite databases with sqlite-vec for vector search
...
export class DocsIndex {
  ...
  constructor(logger: Logger, options?: {
    ...
    this.logger = logger;
    this.baseDir = options?.baseDir ?? join(process.env.HOME ?? "~", ".homarus", "docs");
  ...
  async compile(domain: string, options?: {
    ...
    // 1. Get all non-compiled chunks and their embeddings
    ...
    // 2. Cluster by source file, then merge similar clusters via centroid similarity
    ...
    this.logger.info("Docs compile: clusters formed", { domain, clusters: mergedClusters.length, totalChunks: allChunks.length });

    // 3. Synthesize concept articles via LLM
    let totalArticlesCreated = 0;
    let totalChunksCreated = 0;

    const clusterSummaries = mergedClusters.map((cluster, i) => {
...
Keep it focused and information-dense. No filler.`;
    ...
    return { clustersFound: mergedClusters.length, articlesGenerated: totalArticlesCreated, chunksCreated: totalChunksCreated };
  }
  ...
  async getClusters(domain: string, options?: {
    ...
    // maxClusters cap fix: orphans merge into most similar cluster
    for (const [path, chunks] of groupEntries) {
      if (!mergedPaths.has(path)) {
