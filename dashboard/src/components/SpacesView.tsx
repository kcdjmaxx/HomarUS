  ...
  const handleQuickAdd = async (bucketId: string) => {
    ...
    try {
      await fetch(`${API}/buckets/${bucketId}/items`, {
        ...
        body: JSON.stringify({ title: quickAddTitle, createdBy: "user" }),
      });
      setQuickAddTitle("");
      setQuickAdd(null);
      fetchTree();
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 4px", minHeight: 32 }}>
          ...
          {item.assignee && (
            <span style={{
              ...
              background: item.assignee === "agent" ? (isDark ? "#1a1a3e" : "#ede9fe") : (isDark ? "#1a2e1a" : "#f0fdf4"),
              color: item.assignee === "agent" ? theme.accent : theme.success,
            }}>
