...
The identity system gives a HomarUS agent a persistent sense of self across sessions. It directly affects how the agent processes events, communicates with users, and evolves over time.
...
All identity files live in `~/.homarus/identity/` and are loaded by the `IdentityManager` (`src/identity-manager.ts`). Example templates are provided in `identity.example/`.
...
3. `preferences.md` -- discovered preferences
4. `state.md` -- session continuity
5. Channel-specific overlay (if applicable)
6. Task overlay (if applicable)
7. Workspace files (all `.md` files from the workspace directory)
8. Task prompt (if applicable)
...
- The first 500 characters of soul.md
- Current state summary from state.md (first 200 characters)
...
This file shapes how the agent communicates. A user who prefers terse responses gets different output than one who likes detailed explanations.
...
The purpose is not to make the agent argumentative. It is to make invisible disagreement visible. Most AI assistants silently comply even when the user is making a mistake. This log creates accountability.

## Overlays

Channel-specific overlays modify the agent's behavior per channel. They live in `~/.homarus/identity/overlays/` as `.md` files named after the channel or task:

```
~/.homarus/identity/overlays/
...
A scheduled timer (typically at 8pm) triggers the agent to write a reflection journal entry at `~/.homarus/journal/YYYY-MM-DD.md`. Journal entries are indexed by the memory system via `memory.extraPaths` config, making past reflections searchable.

### Overnight dreams

The dream system runs overnight (typically at 3am) and generates associative, impressionistic entries that serve as overfitting prevention:
...
Dream logs are stored at **0.5x weight** in the memory index with a 7-day decay half-life. They surface subtly in waking searches and fade quickly.
...
2. **Change logging** -- All modifications to identity files should be logged with dates and reasoning.
