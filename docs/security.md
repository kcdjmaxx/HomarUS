...
This document covers HomarUS's security model: permission boundaries, destructive command protection, data isolation, secrets management, and channel trust.

---

## Permission Boundaries

HomarUS operates under a three-tier permission model that governs what the agent can do without human approval.
...
- Search and ingest documentation domains
...
- Impersonate the user in messages
...
- `~/.homarus/` contents (config, secrets, identity, memory) are outside the repo
...
Identity files at `~/.homarus/identity/` have different write permissions:
...
Secrets are stored in `~/.homarus/.env` and loaded automatically by the `Config` class:
...
The config file (`~/.homarus/config.json`) supports `${VAR_NAME}` interpolation, resolved against environment variables at load time:
...
If a variable is not set, it resolves to an empty string.

### Credential Files

OAuth tokens and other structured credentials live in `~/.homarus/secrets/`:
...
The `POST /api/events` endpoint accepts external events from any localhost process. This is intentional -- it allows local services (cron jobs, webhooks, etc.) to inject events. Since it's localhost-only, the trust boundary is the machine itself.
